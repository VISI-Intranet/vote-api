...
import model.ViewedModel
import enumClass.JsonFormatss
import org.json4s.{DefaultFormats, jackson}
import repository.ViewedRepo

import scala.util.{Failure, Success}

object ViewedRoute extends Json4sSupport {
      ...
      concat(
        pathEnd {
          concat(
            (get & parameters("field", "parameter")) {
              (field, parameter) => {
                validate(fields.contains(field),
                  ...
                  onComplete(ViewedRepo.filterViewed(field, parameter)) {
            ...
            get {
              onComplete(ViewedRepo.getAllViewed()) {
            ...
            post {
              entity(as[ViewedModel]) { viewed =>
                onComplete(ViewedRepo.addViewed(viewed)) {
        ...
        path(Segment) { viewedId =>
          concat(
            get {
              onComplete(ViewedRepo.getViewedById(viewedId)) {
            ...
            put {
              entity(as[ViewedModel]) { updatedViewed =>
                onComplete(ViewedRepo.updateViewed(viewedId, updatedViewed)) {
            ...
            delete {
              onComplete(ViewedRepo.deleteViewed(viewedId)) {
